...
# hc = 2000
# lam = 4000
# z0 = lambda x: hc * np.exp(-(x / ac) ** 2) #* np.cos(np.pi * x / lam)**2
# xmap = lambda x, z: xlim * (x - 0.5)
# zmap = lambda x, z: z0(xmap(x, z)) * (1 - z) + z * zlim
...
def initial_condition(xs, ys, solver, pert):
    ...
    u = 10.0 * np.cos(4 * (2 * np.pi * ys / zlim)) * np.cos((2 * np.pi * xs / xlim) + 0.2 * np.pi)
    v = 0 * ys
    ...
    density -= (pert * density / 300) * np.exp(-(2 * (ys - 1.0*rad_max) / rad_max) ** 2) * np.cos(4 * (2 * np.pi * xs / xlim))
...
tends = np.array([0, 100, 200, 300])
# tends = np.array([0, 1, 2, 5])
a = 0.0
upwind=False
...
# plotting
    ...
    fig_list = [plt.subplots(2, 2, sharex=True, sharey=True) for _ in range(len(pfunc_list))]

    labels = ["entropy", "density", "water", "vapour", "liquid", "u", "v"]
