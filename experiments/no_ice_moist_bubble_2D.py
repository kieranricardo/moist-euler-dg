...
ice_solver = ThreePhaseEuler2D(xmap, zmap, poly_order, nx, g=g, cfl=1.0, a=0.5, nz=nz, upwind=True, nprocx=size)
solver = TwoPhaseEuler2D(xmap, zmap, poly_order, nx, g=g, cfl=1.0, a=0.5, nz=nz, upwind=True, nprocx=size)


def initial_condition(xs, ys, solver, ice_solver, pert):
    ...
    qw = ice_solver.rh_to_qw(0.95, p, density)
    qd = 1 - qw

    R = solver.Rd * qd + solver.Rv * qw
    T = p / (R * density)

    assert (qw <= ice_solver.saturation_fraction(T, density)).all()
    ...
    qv = solver.solve_qv_from_entropy(density, qw, s, verbose=False)

    assert np.allclose(qv, qw)
    ql = qw - qv
...
if rank == 0:
    ...
    ice_solver_plot = ThreePhaseEuler2D(xmap, zmap, poly_order, nx, g=g, cfl=0.5, a=0.5, nz=nz, upwind=True, nprocx=1)
    ...
    plot_func_vapour = lambda s: s.project_H1(s.solve_qv_from_entropy(s.h, s.q, s.s)[0] - qv0)
    plot_func_liquid = lambda s: s.project_H1(s.q - s.solve_qv_from_entropy(s.h, s.q, s.s)[1] - ql0)
