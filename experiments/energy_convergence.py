...
for dt in dts:
    ...
    solver = ThreePhaseEuler2D(xmap, zmap, poly_order, nx, g=g, cfl=cfl, a=a, nz=nz, upwind=upwind, nprocx=1)
    u, v, density, s, qw, qv, ql, qi = initial_condition(solver.xs, solver.zs, solver, pert=10.0)
...
plt.savefig(os.path.join(plot_dir, f'conservation_convergence-nx{nx}-nz{nz}-p{poly_order}.png'))
